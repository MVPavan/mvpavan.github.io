...
- For **Static** tile sizes (passed as parameters `[]`), you **MUST** provide a list of descending sizes (e.g., `(4, 1)`) if you want to handle residues (items that don't fit in the main tile). It does _not_ automatically clean up residues.
...
**Purpose**: "Hoists" a boolean check out of a function. Instead of checking `if flag:` inside a hot loop, it generates two completely separate versions of the function—one where `flag` is `True`, one where `False`—and picks the right one _once_ at runtime.
